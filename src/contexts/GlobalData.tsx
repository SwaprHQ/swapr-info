...
export type GlobalData = {
  ...
  totalLiquidityNativeCurrency?: number;
...
async function getGlobalData(factoryAddress, client, blockClient, nativeCurrencyPrice, oldNativeCurrencyPrice) {
  ...
  try {
    ...
    const [oneDayBlock, twoDayBlock, oneWeekBlock, twoWeekBlock] = await getBlocksFromTimestamps(blockClient, [
      utcOneDayBack,
      utcTwoDaysBack,
      utcOneWeekBack,
      utcTwoWeeksBack,
    ...
    if (data && oneDayData) {
      ...
      // TODO: parseFloat expects string, need to change to Math.round
    ...
    if (data && oneWeekData) {
      //@ts-expect-error parseFloat expects string
      // TODO: parseFloat expects string, need to change to Math.round
      const absoluteOneWeekVolumeChange = parseFloat(data.totalVolumeUSD) - parseFloat(oneWeekData.totalVolumeUSD);
...
export function useGlobalData() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const allPairs = await getAllPairsOnSwapr(client);
      updateAllPairsInSwapr(allPairs);

      const allTokens = await getAllTokensOnSwapr(client);
...
export function useGlobalChartData() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const [newChartData, newWeeklyData] = await getChartData(client, oldestDateFetch);
...
export function useGlobalTransactions() {
  ...
  useEffect(() => {
    async function fetchData() {
      if (!transactions) {
        const txns = await getGlobalTransactions(client);
...
export function useNativeCurrencyPrice() {
  ...
  useEffect(() => {
    async function checkForNativeCurrencyPrice() {
      ...
      const [newPrice, oneDayPrice, priceChange] = await getNativeCurrencyPrice(
        client,
        blockClient,
