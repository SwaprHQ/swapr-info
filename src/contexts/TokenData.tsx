...
const getTopTokens = async (client, blockClient, nativeCurrencyPrice, nativeCurrencyPriceOld) => {
  ...
  const oneDayBlock = await getBlockFromTimestamp(blockClient, utcOneDayBack);
  const twoDayBlock = await getBlockFromTimestamp(blockClient, utcTwoDaysBack);
...
const getIntervalTokenData = async (client, blockClient, tokenAddress, startTime, interval = 3600, latestBlock) => {
  ...
  try {
    ...
    for (const row in result) {
      ...
      const derivedNativeCurrency = parseFloat(result[row]?.derivedNativeCurrency);
      if (timestamp) {
        values.push({
          timestamp,
...
export function Updater() {
  ...
  useEffect(() => {
    async function getData() {
      ...
      const topTokens = await getTopTokens(client, blockClient, nativeCurrencyPrice, nativeCurrencyPriceOld);
  ...
  useEffect(() => {
    async function checkForTxns() {
      if (!tokenTxns && allPairsFormatted) {
        const transactions = await getTokenTransactions(client, allPairsFormatted);
...
export function useTokenPairs(tokenAddress) {
  ...
  useEffect(() => {
    async function fetchData() {
      const allPairs = await getTokenPairs(client, tokenAddress);
...
export function useTokenChartData(tokenAddress) {
  ...
  useEffect(() => {
    async function checkForChartData() {
      if (!chartData) {
        const data = await getTokenChartData(client, tokenAddress);
