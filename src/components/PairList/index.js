...
function PairList({ pairs, color, disbaleLinks, maxItems = 10 }) {
  ...
  const ListItem = ({ pairAddress, index }) => {
    ...
    if (pairData && pairData.token0 && pairData.token1) {
      ...
      const apy = formattedPercent((pairData.oneDayVolumeUSD * 0.0025 * 365 * 100) / pairData.reserveUSD)
    ...
    Object.keys(pairs)
      .sort((addressA, addressB) => {
        ...
        if (sortedColumn === SORT_FIELD.APY) {
          const apy0 = parseFloat(pairA.oneDayVolumeUSD * 0.0025 * 356 * 100) / parseFloat(pairA.reserveUSD)
          const apy1 = parseFloat(pairB.oneDayVolumeUSD * 0.0025 * 356 * 100) / parseFloat(pairB.reserveUSD)
