...
export const SUBGRAPH_HEALTH = gql`
  query health {
    indexingStatusForCurrentVersion(subgraphName: "nicoelzer/swapr-rinkeby") {
      synced
      health
      chains {
        chainHeadBlock {
...
export const GLOBAL_CHART = gql`
  query DX($startTime: Int!, $skip: Int!) {
    dxswapDayDatas(first: 1000, skip: $skip, where: { date_gt: $startTime }, orderBy: date, orderDirection: asc) {
      id
      date
      totalVolumeUSD
      dailyVolumeUSD
...
export const GLOBAL_DATA = block => {
  const queryString = ` query dxswapFactories {
    dxswapFactories(
