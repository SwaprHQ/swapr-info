...
function TxnList({ transactions, symbol0Override, symbol1Override, color }: TxnListProps) {
      ...
      if (transactions.mints.length > 0) {
        transactions.mints.map((mint) => {
          const newTxn: Transaction = {
            ...
            account: mint.to,
            token0Symbol: updateNameData(mint.pair).token0.symbol,
            token1Symbol: updateNameData(mint.pair).token1.symbol,
      ...
      if (transactions.burns.length > 0) {
        transactions.burns.map((burn) => {
          const newTxn: Transaction = {
            ...
            account: burn.sender,
            token0Symbol: updateNameData(burn.pair).token0.symbol,
            token1Symbol: updateNameData(burn.pair).token1.symbol,
