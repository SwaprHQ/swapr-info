...
interface FarmingListProps {
  campaigns: any;
  color?: string;
  maxItems?: number;
}

export default function FarmingList({ campaigns, color, maxItems = 10 }: FarmingListProps) {
    ...
    Object.keys(campaigns)
      ...
      .map((pairIndex, index) => {
        return (
          pairIndex && (
            <div key={index}>
              <ListItem key={index} index={(page - 1) * ITEMS_PER_PAGE + index + 1} pairAddress={pairIndex} />
              <Divider />
            </div>
          )
        );
